...
const I = {
  ...
  // Xiaomi-style Tasks top tab (checkbox)
  tasksTab:
  ...
  settingsTab:
    '<path d="M12 2l7.5 4.3v8.7L12 22 4.5 17V6.3L12 2z"/>'
...
const KILL = new Set([
  ...
  'LINK', 'META', 'FORM', 'INPUT', 'BUTTON', 'VIDEO',
  'AUDIO', 'SVG', 'IMG'
]);
...
function sanitize(html) {
  ...
  for (const el of Array.from(t.querySelectorAll('*'))) {
    ...
    for (const a of Array.from(el.attributes)) {
      const ok =
        (el.tagName === 'A' && a.name.toLowerCase() === 'href' && /^(https?|mailto):/i.test(a.value || ''))
  ...
  const inline = node => {
    ...
    node.childNodes.forEach(c => {
      if (c.nodeType === Node.TEXT_NODE) out += c.textContent;
        ...
        else if (tag === 'SPAN' && c.classList.contains('cb')) { /* handled */ }
        else out += inline(c);
      }
    });
    return out;
  ...
  blocks.forEach(el => {
    ...
    if (tag === 'H1') md += '# ' + inline(el) + '\n\n';
      ...
      Array.from(el.children).forEach(li => {
        md += '- [' + (li.classList.contains('done') ? 'x' : ' ') + '] ' + inline(li).trim() + '\n';
      });
      md += '\n';
    }
    else if (tag === 'UL') {
...
function mdToHtml(md) {
  ...
  for (const line of lines) {
    ...
    if ((m = line.match(/^\s*[-*]\s+\[( |x|X)\]\s+(.*)$/))) {
      ...
      html += '<li' + (m[1].toLowerCase() === 'x' ? ' class="done"' : '') + '>'
...
if (!Array.isArray(db.notebooks) || !db.notebooks.length) {
  db.notebooks = seedNB();
}

db.notebooks.forEach(nb => {
  if (nb.open === undefined) nb.open = false;
});
...
function save() {
  try {
    ...
    window.dispatchEvent(new CustomEvent('take-fast-notes-storage-error'));
    return false;
  }
}
...
function notesIn(id) {
  ...
  return db.notes.filter(n => ids.has(n.nb) && !n.trashed);
}
